...
<?php
 ...
 $sql = "INSERT INTO users(firstname,lastname,email,phone,username,password) VALUES ('$firstname','$lastname','$email','$phone','$inputuser','$inputpass')";

 if($con->query($sql)=== TRUE){
	 echo "<br/><br/><br/><br/><br/> Successfully Registered";
     ...
     echo "<div class=welcome>";
       ...
       echo " Welcome <span class=user>". $_SESSION['userName'] ;" </span>";
        ...
        $sql = "SELECT firstname,lastname,email,phone,username,password FROM users WHERE username='$_SESSION[userName]'";
        $result = $mysqli->query($sql); 
        
       echo "<div id='registered'>";
       echo "<span><h2>Your Profile:<h2></span>";
        ...
        while($row = $result->fetch_assoc()){ 
            ...
            echo "<tr><td>$row[firstname]</span></td><td>$row[lastname]</span></td><td><div class='userlist'><span>$row[email]</span>";"</td>";
             echo "<td>$row[phone]<td>$row[username]<td>$row[password]</td></div>";"</td></tr></table>";
        }
       // ?  
        echo "</div>";
 }
...
<div>
		<table align=center>
		<tr>
		<td><button type="submit" class="btn btn-primary"><a href="userupdate.php">Update Profile</a></button></td><td><button type="submit" class="btn btn-primary"><a href="book.html">Book Tickets</a></button></td>
		</tr>
        </table>

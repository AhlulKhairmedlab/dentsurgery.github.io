       ...
       <br/><br/><br/><br/><br/>
        ...
        while($row = $result->fetch_assoc()){ //returns associative array of fetched row
            ...
            echo "<tr><td>$row[firstname]</span></td><td>$row[lastname]</span></td><td><div class='userlist'><span>$row[email]</span>";"</td>";
             echo "<td>$row[phone]<td>$row[username]<td>$row[password]</td></div>";"</td></tr></table><br/>";
        }
        ?>  
        </div>
		<div>
		<table align=center>
		<tr>
		<td><button type="submit" class="btn btn-primary"><a href="userupdate.php">Update Profile</a></button></td><td><button type="submit" class="btn btn-primary"><a href="userbook.php">Book Tickets</a></button></td>
		</tr>
        </table>
